...
def build_pcs() -> None:
    ...
    run_pipeline = nbf.v4.new_code_cell(
        ...
        "})\n"
        "preds['qty_total_forecast'] = preds['qty_total_forecast'].fillna(preds['qty_total_baseline'])\n"
        "preds['qty_total_forecast'] = preds.apply(\n"
        "    lambda row: row['qty_total_baseline'] if row['qty_total_forecast'] <= 0 else row['qty_total_forecast'],\n"
        "    axis=1\n"
        ")\n"
        "summary_report = pd.DataFrame({\n"
        "    'group_key': [s.group_key[0] for s in summaries],\n"
